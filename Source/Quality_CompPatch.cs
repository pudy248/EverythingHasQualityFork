...
namespace QualityEverything
{
	...
	public class Quality_CompPatch
	{
		...
		public static void DefPatch()
		{
			...
			ThingDef def;
			bool hasComp;
			List<CompProperties> compsToAdd = new List<CompProperties>();
			CompProperties comp1 = new CompProperties();
			comp1.compClass = typeof(CompQuality);
			compsToAdd.Add(comp1);

			if (ModLister.HasActiveModWithName("Improve This"))
			{
				CompProperties comp2 = new CompProperties();
				comp2.compClass = AccessTools.TypeByName("RimWorld___Improve_This.ImproveThisComp");
				compsToAdd.Add(comp2);
			}
			...
			for (int m = 0; m < DefDatabase<ThingDef>.AllDefsListForReading.Count; m++)
			{
				...
				else if (def.IsStuff)
				{
					if (ModSettings_QEverything.indivStuff)
					{
						...
						if (!hasComp && ModSettings_QEverything.stuffDict[def.defName] == true) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
						else if (hasComp && ModSettings_QEverything.stuffDict[def.defName] == false) foreach(CompProperties compProps in compsToAdd) def.comps.Remove(compProps);
					}
					else if (!hasComp && ModSettings_QEverything.stuffQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
				...
				else if (def.building != null)
				{
					...
					else if (ModSettings_QEverything.indivBuildings)
					{
						...
						if (!hasComp && ModSettings_QEverything.bldgDict[def.defName] == true) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
						else if (hasComp && ModSettings_QEverything.bldgDict[def.defName] == false) foreach(CompProperties compProps in compsToAdd) def.comps.Remove(compProps);
					}
					else if (hasComp) continue;
					else if (def.IsWorkTable)
					{
						if (ModSettings_QEverything.workQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
					}
					else if (def.IsWithinCategory(ThingCategoryDef.Named("BuildingsSecurity")) || def.building.IsTurret)
					{
						if (ModSettings_QEverything.securityQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
					}
					else if (ModSettings_QEverything.edificeQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
				}
				else if (!def.IsIngestible && (def.IsWeapon || def.IsShell || def.IsWithinCategory(ThingCategoryDef.Named("Grenades"))))
				{
					if (ModSettings_QEverything.indivWeapons)
					{
						...
						if (!hasComp && ModSettings_QEverything.weapDict[def.defName] == true) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
						else if (hasComp && ModSettings_QEverything.weapDict[def.defName] == false) foreach(CompProperties compProps in compsToAdd) def.comps.Remove(compProps);
					}
					else if (hasComp) continue;
					else if (def.IsShell && ModSettings_QEverything.shellQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
					else if (def.IsWeapon && ModSettings_QEverything.weaponQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
				}
				else if (def.IsApparel)
				{
					if (ModSettings_QEverything.indivApparel)
					{
						...
						if (!hasComp && ModSettings_QEverything.appDict[def.defName] == true) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
						else if (hasComp && ModSettings_QEverything.appDict[def.defName] == false) foreach(CompProperties compProps in compsToAdd) def.comps.Remove(compProps);
					}
					else if (!hasComp && ModSettings_QEverything.apparelQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
				}
				else if (ModSettings_QEverything.indivOther)
				{
					...
					if (!hasComp && ModSettings_QEverything.otherDict[def.defName] == true) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
					else if (hasComp && ModSettings_QEverything.otherDict[def.defName] == false) foreach(CompProperties compProps in compsToAdd) def.comps.Remove(compProps);
				}
				else if (hasComp) continue; //Avoids duplicate comps
				else if (def.IsDrug && ModSettings_QEverything.drugQuality)
				{
					foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
				}
				else if (def.IsMedicine)
				{
					if (ModSettings_QEverything.medQuality) foreach(CompProperties compProps in compsToAdd) def.comps.Add(compProps);
				}
